...
export class GridLayer extends Layer {
  ...
  setup(onLoaded: CallableFunction) {
    ...
    this.loadTiles(
      this.context.navigation.target,
      this.context.navigation.position
    );
    onLoaded();
  }
  ...
  initPlaceholders() {
    ...
    for (let tile of this.layout.tiles) {
      ...
      const placeholder = MetacityGL.Graphics.Models.TileModel.create({
        center: [
          (tile.x + 0.5) * this.layout.tileWidth,
          (tile.y + 0.5) * this.layout.tileHeight,
          0,
        ],
  ...
  loadTiles(target: vec3, position: vec3) {
    ...
    this.layout.tiles.forEach((tile) => {
      const d = GridLayer.dist(
        tile.x * this.layout!.tileWidth + dx,
        tile.y * this.layout!.tileHeight + dy,
        target.x,
        target.y
      );
